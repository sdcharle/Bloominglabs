...
In general
Use linereceiver
send comm to all cli connected to server
use 'usage'
use daemon facilities
...
class RFIDClientFactory(Factory):
    ...
    def notifyAll(self, data):
        log.msg("RFID Client: let's notify all up in this bitch")
...
if __name__ == '__main__':
   ...
   log.msg("Serial port be: %s" % options.SERIAL_PORT)
   log.msg("Baud be: %s" % options.BAUD)
...
# should catch serial exception and bail possibly.

   sp = SerialPort(USBClient(tcpfactory), options.SERIAL_PORT, reactor, baudrate=options.BAUD)
...
application = service.Application("rfid_server")

# this hooks the collection we made to the application
top_service.setServiceParent(application)

2 - simpler
application = service.Application('rfid', uid=1, gid=1)
...
listen TCP vs TCPServer (just try both!)
